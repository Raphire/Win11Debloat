BeforeAll {
    ...
    function GetInstalledAppsViaWinget { param($TimeOut, [switch]$NonBlocking) @() }
    ...
    . (Join-Path $PSScriptRoot '..\Scripts\AppRemoval\RemoveApps.ps1')
}

Describe 'RemoveApps workflow' {
    BeforeEach {
        $script:Params = @{}
        ...
        Mock GetInstalledAppsViaWinget { @() }
    ...
    It 'honors WhatIf without invoking either removal backend' {
        ...
        RemoveApps -appsList @('One.App', 'Two.App')
    ...
    It 'dispatches each app to its configured backend and target scope' {
        ...
        RemoveApps -appsList @('Winget.App', 'Appx.App')
    ...
    It 'prompts for forced Edge removal at most once after failed winget removals' {
        ...
        RemoveApps -appsList @('Microsoft.Edge', 'XPFFTQ037JWMHS')
...
Describe 'Remove-AppxApp dispatch' {
...
Describe 'Test-AppStillInstalled' {
    ...
    It 'prefers all-user Appx detection and avoids winget lookup' {
        ...
        Should -Invoke GetInstalledAppsViaWinget -Times 0 -Exactly
    }
