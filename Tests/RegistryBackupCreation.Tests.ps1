BeforeAll {
    ...
    . (Join-Path $PSScriptRoot '..\Scripts\Features\BackupRegistryFeatureSelection.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\SaveToFile.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\Features\BackupRegistryState.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\Features\BackupRegistrySnapshotCapture.ps1')
}
...
Describe 'New-RegistrySettingsBackup' {
    ...
    It 'creates the backup directory and saves a generated payload' {
        ...
        Should -Invoke SaveToFile -Times 1 -Exactly -ParameterFilter { $FilePath -eq $result -and $MaxDepth -eq 25 }
    }
...
Describe 'registry backup capture-plan helpers' {
