BeforeAll {
    ...
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\LoadJsonFile.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\Helpers\AddParameter.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\SaveToFile.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\LoadSettings.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\SaveSettings.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\LoadAppsFromFile.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\LoadAppsDetailsFromJson.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\LoadAppPresetsFromJson.ps1')
    . (Join-Path $PSScriptRoot '..\Scripts\FileIO\ValidateAppslist.ps1')
...
Describe 'LoadSettings' {
    ...
    It 'loads enabled, known, compatible settings and skips all other entries' {
        LoadSettings -filePath (Join-Path $script:JsonFixturePath 'DefaultSettings.Valid.json')

        Should -Invoke AddParameter -Times 1 -Exactly -ParameterFilter { $parameterName -eq 'Supported' -and $value -eq 'configured' }
    }
    ...
    It 'throws when <Case>' -ForEach @(
...

        {
            & {
                ...
                LoadSettings -filePath $path
            }
        } | Should -Throw "Failed to load settings from $FileName"
        Should -Invoke AddParameter -Times 0 -Exactly
...
Describe 'SaveSettings' {
    ...
    It 'saves only feature parameters and excludes control or unknown parameters' {
        ...
        Should -Invoke SaveToFile -Times 1 -Exactly -ParameterFilter {
    ...
    It 'does not persist in WhatIf mode' {
        ...
        Should -Invoke SaveToFile -Times 0 -Exactly
    }

    It 'reports a persistence failure without throwing' {
        $script:Params = @{ Feature = $true }
        Mock SaveToFile { $false }

        { SaveSettings } | Should -Not -Throw
...
Describe 'LoadAppsFromFile' {
    It 'returns selected IDs, trims whitespace, and supports scalar and array AppId values' {
        ...
        @(LoadAppsFromFile -appsFilePath $path) | Should -Be @('One.App', 'Two.App')
    }

    It 'returns an empty collection for a missing file' {
        @(LoadAppsFromFile -appsFilePath (Join-Path $TestDrive 'missing.json')) | Should -HaveCount 0
    }
    ...
    It 'reports invalid JSON and invokes the CLI acknowledgement hook' {
        ...
        @(LoadAppsFromFile -appsFilePath $path) | Should -HaveCount 0
        Should -Invoke Write-Error -Times 1 -Exactly
        Should -Invoke AwaitKeyToExit -Times 1 -Exactly
    }
}

Describe 'ValidateAppslist' {
    It 'normalizes wildcard selections and skips unsupported applications' {
        Mock LoadAppsDetailsFromJson { @([PSCustomObject]@{ AppId = @('One.App', 'Two.App') }) }
        Mock Write-Host {}

        @(ValidateAppslist -appsList @('*One.App*', ' Missing.App ')) | Should -Be @('One.App')
        Should -Invoke Write-Host -Times 1 -Exactly -ParameterFilter { $Object -like "*Missing.App*" }
    }
}

Describe 'LoadAppsDetailsFromJson and LoadAppPresetsFromJson' {
    BeforeEach {
    ...
    It 'projects app metadata, defaults removal method, and honors initial checked state' {
        $apps = @(LoadAppsDetailsFromJson -InitialCheckedFromJson)
    ...
    It 'filters to installed apps using Appx and winget detection' {
        ...
        $apps = @(LoadAppsDetailsFromJson -OnlyInstalled -InstalledList @())
    ...
    It 'loads presets and preserves their ID arrays' {
        $presets = @(LoadAppPresetsFromJson)
    ...
    It 'returns empty collections and reports malformed JSON' {
        ...
        @(LoadAppsDetailsFromJson) | Should -HaveCount 0
        @(LoadAppPresetsFromJson) | Should -HaveCount 0
        Should -Invoke Write-Error -Times 1 -Exactly
